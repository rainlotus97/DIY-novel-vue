...
module.exports = {
  ...
  // 部署应用包时的基本URL，如果是生产环境，部署到 / 路径；如果是开发环境，部署到根路径
  publicPath: !debug
    ? './'
    : './',
  ...
  css: {
    loaderOptions: {
      sass: {
        prependData: `@import "@/assets/main.scss";` //引入全局变量   
      }
    }
  },
  ...
  devServer: {
    ...
    proxy: {
      ...
      // '/api/*': {
      //   target: 'http://47.100.139.104:8899/', // 目标服务器地址
      ...
      // },
    '/book-list/*': {
      target: 'https://api.zhuishushenqi.com', // 目标服务器地址
      secure: false,                    // 目标服务器地址是否是安全协议
      changeOrigin: true,               // 是否修改来源, 为true时会让目标服务器以为是webpack-dev-server发出的请求!服务端和服务端的请求是没有跨域的
      pathRewrite: { '/book-list': '/' }     // 将/api开头的请求地址, /api 改为 /, 即 /api/xx 改为 /xx
    }
  }
},
